...
def run_main_ui(screen, background_image, logo_image, font_path):
    try:
        ...
        quit_button_rect = pygame.Rect(screen.get_width() - 35, 10, 20, 20)
        pygame.draw.rect(screen, RED2, quit_button_rect, border_radius=10)
        draw_text(screen, "X", quit_button_rect.center, font_size=20, color=RED3)

        minimize_button_rect = pygame.Rect(screen.get_width() - 62, 10, 20, 20)
        pygame.draw.circle(screen, YELLOW2, minimize_button_rect.center, 10)
        draw_text(screen, "-", minimize_button_rect.center, font_size=35, color=YELLOW3)
...
def draw_ui(screen):
    ...
    quit_button_rect = pygame.Rect(screen.get_width() - 35, 10, 20, 20)
    pygame.draw.rect(screen, (243, 90, 86, 150), quit_button_rect, border_radius=10)
    draw_text(screen, "X", quit_button_rect.center, font_size=20, color=(126, 5, 8))
...
class App:
    def __init__(self, name, rect, position, color=(107, 107, 107)):
        self.name = name
        self.rect = rect
    ...
    def load_icon_image(self):
        if self.name == "SeanUI":
            ...
            icon_path = "App icon/IMG_8851.PNG"
    ...
    def draw(self, surface):
        ...
        draw_text(surface, self.name, self.position, font_size=15, color=(107, 107, 107))
...
def create_apps(app_count, bottom_bar_rect):
    ...
    for i in range(app_count):
        ...
        if i == 0:
            ...
            app = App("Terminal", app_rect, _position, color=(0, 0, 0))
        apps.append(app)
    return apps
...
def open_app(app_name):
    ...
    app_paths = {
        ...
        "Terminal": "App code/codeterminal-1.py",
    }
